...
main() {
  ...
  final uploadLocation = "https://example.com/tus/1ae64b4f-bd7a-410b-893d-3614a4bd68a6";
  final urlWithPort = Uri.parse("https://example.com:1234/tus");
  final uploadLocationWithPort = "https://example.com:1234/tus/1ae64b4f-bd7a-410b-893d-3614a4bd68a6";
  ...
  test("client_test.TusClient().metadata.filename", () async {
    final client = MockTusClient(url, file, metadata: {"id": "sample", "filename": "another-name.txt"});

    // expect(client.fingerprint, isNot("test.txt"));
    expect(client.fingerprint, endsWith("test.txt"));
    expect(client.uploadMetadata, matches(RegExp(r"filename YW5vdGhlci1uYW1lLnR4dA==")));
    expect(client.uploadMetadata, matches(RegExp(r"id c2FtcGxl")));
  });
  ...
  test('client_test.TusClient.create()', () async {
    ...
    when(client.httpClient?.post(url, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 201, headers: {"location": uploadLocation}));

    await client.createUpload();

    expect(client.uploadUrl.toString(), equals(uploadLocation));
    expect(verify(client.httpClient?.post(url, headers: captureAnyNamed('headers'))).captured.first,
        contains('Tus-Resumable'));
  });

  test('client_test.TusClient.create().no.scheme', () async {
    final client = MockTusClient(url, file);
    when(client.httpClient?.post(url, headers: anyNamed('headers'))).thenAnswer((_) async =>
        http.Response("", 201, headers: {"location": "//example.com/tus/1ae64b4f-bd7a-410b-893d-3614a4bd68a6"}));
  ...
  test('client_test.TusClient.create().no.host', () async {
    ...
    when(client.httpClient?.post(url, headers: anyNamed('headers'))).thenAnswer(
        (_) async => http.Response("", 201, headers: {"location": "/tus/1ae64b4f-bd7a-410b-893d-3614a4bd68a6"}));
  ...
  test('client_test.TusClient.create().no.host.with.port', () async {
    ...
    when(client.httpClient?.post(urlWithPort, headers: anyNamed('headers'))).thenAnswer(
        (_) async => http.Response("", 201, headers: {"location": "/tus/1ae64b4f-bd7a-410b-893d-3614a4bd68a6"}));

    await client.createUpload();

    expect(client.uploadUrl.toString(), equals(uploadLocationWithPort));
    expect(verify(client.httpClient?.post(urlWithPort, headers: captureAnyNamed('headers'))).captured.first,
        contains('Tus-Resumable'));
  });
  ...
  test('client_test.TusClient.create().double.header', () async {
    final client = MockTusClient(url, file);
    when(client.httpClient?.post(url, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 201, headers: {"location": "$uploadLocation,$uploadLocation"}));
  ...
  test('client_test.TusClient.create().failure.empty.location', () async {
    final client = MockTusClient(url, file);
    when(client.httpClient?.post(url, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 201, headers: {"location": ""}));

    expectLater(
        () => client.createUpload(),
        throwsA(predicate(
            (e) => e is ProtocolException && e.message == 'missing upload Uri in response for creating upload')));
  });
  ...
  test('client_test.TusClient.createUpload().failure.server.error', () async {
    ...
    expectLater(
        () => client.createUpload(),
        throwsA(predicate(
            (e) => e is ProtocolException && e.message == 'unexpected status code (500) while creating upload')));
  });
  ...
  test('client_test.TusClient.upload()', () async {
    ...
    when(client.httpClient?.head(any, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 200, headers: {"upload-offset": "0"}));
    when(client.httpClient?.patch(
      any,
      headers: anyNamed('headers'),
      body: anyNamed('body'),
    )).thenAnswer((_) async => http.Response("", 204, headers: {"upload-offset": "100"}));
    ...
    expect(verify(client.httpClient?.post(any, headers: captureAnyNamed('headers'))).captured.first,
        contains('Tus-Resumable'));
    expect(verify(client.httpClient?.head(any, headers: captureAnyNamed('headers'))).captured.first,
        contains('Tus-Resumable'));
    expect(
        verify(client.httpClient?.patch(any, headers: captureAnyNamed('headers'), body: anyNamed('body')))
            .captured
            .first,
        contains('Tus-Resumable'));
  });
  ...
  test('client_test.TusClient.upload().double.header', () async {
    ...
    when(client.httpClient?.post(url, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 201, headers: {"location": uploadLocation}));
    when(client.httpClient?.head(any, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 200, headers: {"upload-offset": "0,0"}));
    when(client.httpClient?.patch(
      any,
      headers: anyNamed('headers'),
      body: anyNamed('body'),
    )).thenAnswer((_) async => http.Response("", 204, headers: {"upload-offset": "100,100"}));
  ...
  test('client_test.TusClient.upload().offset.failure.server.error', () async {
    ...
    expectLater(
        ...
        throwsA(predicate(
            (e) => e is ProtocolException && e.message == 'unexpected status code (500) while resuming upload')));
  });
  ...
  test('client_test.TusClient.upload().offset.failure.missing', () async {
    ...
    when(client.httpClient?.post(url, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 201, headers: {"location": uploadLocation}));
    when(client.httpClient?.head(any, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 204, headers: {"upload-offset": ""}));
    ...
    expectLater(
        ...
        throwsA(predicate(
            (e) => e is ProtocolException && e.message == 'missing upload offset in response for resuming upload')));
  });
  ...
  test('client_test.TusClient.upload().patch.failure.server.error', () async {
    ...
    when(client.httpClient?.post(url, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 201, headers: {"location": uploadLocation}));
    when(client.httpClient?.head(any, headers: anyNamed('headers')))
        .thenAnswer((_) async => http.Response("", 204, headers: {"upload-offset": "0"}));
    when(client.httpClient?.patch(
      any,
      headers: anyNamed('headers'),
      body: anyNamed('body'),
    ...
    expectLater(
        ...
        throwsA(predicate(
            (e) => e is ProtocolException && e.message == 'unexpected status code (500) while uploading chunk')));
  });
  ...
  test('client_test.TusClient.upload().patch.failure.missing.offset', () async {
    ...
    expectLater(
        ...
        throwsA(predicate((e) =>
            ...
            e.message == 'response to PATCH request contains no or invalid Upload-Offset header')));
  });
  ...
  test('client_test.TusClient.upload().patch.failure.wrong.offset', () async {
    ...
    when(client.httpClient?.patch(
      ...
      headers: anyNamed('headers'),
      body: anyNamed('body'),
    )).thenAnswer((_) async => http.Response("", 204, headers: {"upload-offset": "50"}));
    ...
    expectLater(
        ...
        throwsA(predicate((e) =>
            e is ProtocolException &&
            e.message == 'response contains different Upload-Offset value (50) than expected (100)')));
  });
}
